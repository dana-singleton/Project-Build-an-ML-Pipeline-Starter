...
import mlflow
import mlflow.sklearn  # <-- make sure this is imported
...
def go(args):
    ...
    run = wandb.init(
        project="nyc_airbnb",
        entity="dsin190-western-governors-university",
    ...
    # ---- LOG METRICS (this is what you need!) ----
    run.summary["r2"] = r_squared
    run.summary["mae"] = mae

    # also log as regular metrics so “Charts” stops saying “no metrics logged”
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # Non-ordinal: impute + one-hot
    non_ordinal_categorical_preproc = make_pipeline(
        SimpleImputer(strategy="most_frequent"),
        OneHotEncoder(handle_unknown="ignore"),
    )

    # Numerical (zero-imputed)
    zero_imputed = [
        "minimum_nights",
        "number_of_reviews",
        "reviews_per_month",
